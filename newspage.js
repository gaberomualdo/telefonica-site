...
function addArticle(newsArticle){
  ...
  var className = "other";
  if(newsArticle.title && (newsArticle.title.toLowerCase().indexOf("telefonica") != -1 || newsArticle.title.toLowerCase().indexOf("O2") != -1)){
    className = "vodafoneTitle";
  }else if(newsArticle.description && (newsArticle.description.toLowerCase().indexOf("telefonica") != -1 || newsArticle.description.toLowerCase().indexOf("O2") != -1)){
...
requestAPI("https://newsapi.org/v2/everything?language=en&pageSize=100&q=telefonica&apiKey=377facbb80c140388ec9cdb16a7f7ee0", function(responseText){
